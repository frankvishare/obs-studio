...
VISHAREOutput::VISHAREOutput(obs_data_t *, obs_output_t *output)
	: output(output),
	  endpoint_url(),
	  bearer_token(),
	  resource_url(),
...
bool VISHAREOutput::Start()
{
	...
	if (start_stop_thread.joinable())
		start_stop_thread.join();
	start_stop_thread = std::thread(&VISHAREOutput::StartThread, this);

	return true;
}

void VISHAREOutput::Stop(bool signal)
{
	std::lock_guard<std::mutex> l(start_stop_mutex);
	if (start_stop_thread.joinable())
		start_stop_thread.join();
...
void VISHAREOutput::ConfigureAudioTrack(std::string media_stream_id,
				     std::string cname)
{
...
bool VISHAREOutput::Connect()
{
	...
	if (location_header_count < static_cast<size_t>(redirect_count) + 1) {
		do_log(LOG_ERROR,
		       "VISHARE server did not provide a resource URL via the Location header");
		cleanup();
		obs_output_signal_stop(output, OBS_OUTPUT_CONNECT_FAILED);
		return false;
	}
	...
	if (rc) {
		do_log(LOG_ERROR,
		       "VISHARE server provided a invalid resource URL via the Location header");
		cleanup();
		obs_output_signal_stop(output, OBS_OUTPUT_CONNECT_FAILED);
		return false;
	}
	...
	resource_url = url;
	curl_free(url);
	do_log(LOG_DEBUG, "VISHARE Resource URL is: %s", resource_url.c_str());
	...
	try {
		...
		do_log(LOG_ERROR, "VISHARE server responded with invalid SDP: %s",
		       err.what());
		cleanup();
...
void VISHAREOutput::StartThread()
{
	if (!Init())
		return;
...
void register_vishare_output()
{
	...
	info.create = [](obs_data_t *settings, obs_output_t *output) -> void * {
		return new VISHAREOutput(settings, output);
	};
	info.destroy = [](void *priv_data) {
		delete static_cast<VISHAREOutput *>(priv_data);
	};
	info.start = [](void *priv_data) -> bool {
		return static_cast<VISHAREOutput *>(priv_data)->Start();
	};
	info.stop = [](void *priv_data, uint64_t) {
		static_cast<VISHAREOutput *>(priv_data)->Stop();
	};
	info.encoded_packet = [](void *priv_data,
				 struct encoder_packet *packet) {
		static_cast<VISHAREOutput *>(priv_data)->Data(packet);
	};
